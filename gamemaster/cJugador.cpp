...
void cJugador::distribucionTropasEnPaises(cListaT<cTropa>* listaTropas) {
	...
	if (listaTropas != NULL)
	{
		for (int i = 0; i < listaTropas->getCA(); i++) //para cada tropa
		{
			system("cls");
			cout << "JUGADOR " << ((int)jugador + 1) << endl;
			cout << "Donde desea agregar la siguiente tropa?\n" << (*listaTropas)[i] << endl; //le preguntamos al jugador
			for (int j = 0; j < listaPaises->getCA(); j++) //en qué país la quiere agregar
			{
			...
			if (paisptr == NULL) {
				i--;
				continue; //hasta que ingrese una respuesta válida
			}
			paisptr->agregarTropas((*listaTropas)[i]); //y agregamos la tropa
		}
	}
}
